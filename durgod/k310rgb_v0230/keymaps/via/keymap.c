...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    /* Keymap _BASE: Base Layer (Default Layer)
      ...
      * └────┴────┴────┴────────────────────────┴────┴────┴────┴────┘ └───┴───┴───┘ └───────┴───┴───┘
        KC_ESC,           KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,     KC_PSCR, KC_SCRL, KC_PAUS,   KC_MUTE  or RGB_MOD  or  LT(1,KC_WWW_BACK),
...
*/
     [WIN_FN] = LAYOUT_all(/* Function Layer */
        INIT_3S,          KC_BRID, KC_BRIU, KC_TASK, KC_FLXP,  KC_DIC, KC_MSEL, KC_MPRV, KC_MPLY, KC_MNXT, KC_WHOM, KC_CALC,   KC_WSCH,   KC_WBAK, KC_WFWD, KC_LOCK2,                           RGB_MOD,
        ...
        _______, _______, KC_WINM, _______, INIT_3S, _______, _______, QK_BOOT, _______, _______, _______, _______, _______,   _______,   RGB_SAD, RGB_HUD, RGB_RMOD,     KC_BTN4, KC_MS_U, KC_BTN5, KC_WH_U,
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,   _______,                                   KC_MS_L, KC_BTN3, KC_MS_R,
        _______, _______, _______, _______,  EE_CLR, _______, KC_S_LED, _______, KC_MACM,  _______, _______, _______,           _______,            RGB_VAI,               KC_WH_L, KC_MS_D, KC_WH_R, KC_WH_D, 
        _______, KC_TGUI, _______,                             _______,                             KC_PC1, _______, KC_PC2,    KC_TPC,   RGB_SPD, RGB_VAD, RGB_SPI,      KC_BTN1,       KC_BTN2),
/*                 
...
*/
    ...
    [MAC_FN]   = LAYOUT_all(/* Layer 4 */
        INIT_3S,          KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,     KC_F13,  KC_F14,  KC_F15,                            RGB_MOD,
        DM_RSTP, DM_REC1, DM_REC2, DM_PLY1, DM_PLY2, _______, _______, _______, _______, _______, _______, NK_OFF,   NK_ON,  RGB_TOG,    RGB_SAI, RGB_HUI, RGB_MOD,     KC_CALC, KC_ACL0, KC_ACL1, KC_ACL2, 
        _______, _______, KC_WINM, _______, INIT_3S, _______, _______, QK_BOOT, _______, _______, _______, _______, _______, _______,    RGB_SAD, RGB_HUD, RGB_RMOD,    KC_BTN4, KC_MS_U, KC_BTN5, KC_WH_U, 
        _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,                                   KC_MS_L, KC_BTN3, KC_MS_R, 
        _______, _______, _______, _______,  EE_CLR, _______, KC_S_LED, _______, KC_MACM, _______, _______, _______,          _______,             RGB_VAI,              KC_WH_L, KC_MS_D, KC_WH_R, KC_WH_D,  
        _______, _______, KC_TGUI,                         _______,                                KC_PC1, _______, KC_PC2,   KC_TPC,    RGB_SPD, RGB_VAD, RGB_SPI,     KC_BTN1,       KC_BTN2)
    };
//-----------------------------------------------------------------
...
// #define NUM_ENCODERS 2
    ...
    // [WIN_FN]   = { ENCODER_CCW_CW(LCTL(KC_PMNS), LCTL(KC_PPLS)) },
    ...
    switch (keycode) {
        case INIT_3S:
            if (record->event.pressed){
                ...
                INIT3S_on   = false;
                bootloader_jump();      //3이상 안누르면 펌웨어 업그레이드 (ESC키에 적합, R카에는 안적함)
                // return false;              
            }
            break;
...
bool rgb_matrix_indicators_user(void)  {
    ...
    if (host_leds.caps_lock) { 
        rgb_matrix_set_color(75, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);                     // 0X20            
    }
    if (host_leds.scroll_lock) {
        rgb_matrix_set_color(14, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);       // 0X20
    }        
    if (host_keyboard_led_state().num_lock) {
        rgb_matrix_set_color(19, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);    
    }
    ...
    if (isWinLock){         // WIN key Lock LED Blink
        ...
        if (isWinLockLedOn) {
            if (isMacMode) rgb_matrix_set_color(110, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);
            else  rgb_matrix_set_color(111, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);
        }
    }

    if (isRecordingLedOn) {                 // Dynamic Macro Recording LED Blink 
        rgb_matrix_set_color(37, Status_LED_Bright_value, Status_LED_Bright_value, Status_LED_Bright_value);            
    }
    if (bootloader_mode) { 
...
// rgb_matrix_set_color(pgm_read_byte(&convert_led_location2number[11]),  RGB_RED);         //  RGB_TOG  <- too heavy.
    switch (layer) {
        case 0:
        case 1:
            if (kvm_pc_sel)  rgb_matrix_set_color(106,0,0,Status_LED_Bright_value);        // Windows Mode & PC2
            else            rgb_matrix_set_color(108,0,0,Status_LED_Bright_value);        // Windows Mode & PC1
            break;   

        case 2:
        case 3:
            if (kvm_pc_sel)  rgb_matrix_set_color(106,Status_LED_Bright_value,0,0);      // MAC Mode & PC2
            else            rgb_matrix_set_color(108,Status_LED_Bright_value,0,0);      // MAC Mode & PC2
            break;          
    }
    return TRUE;
}
