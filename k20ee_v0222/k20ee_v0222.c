...
#include "k20ee_v0222.h"
#include <ch.h>
#include <hal.h>

// PIN command = writePinLow(PIN), writePinHigh(PIN), setPinOutput(PIN), setPinInputHigh(PIN) writePin(PIN, !true),
// readPin(PIN), togglePin(PIN),
...
//    writePinHigh(LED_SCROLL_LOCK_PIN);
    writePinHigh(LED_e0_R);                
    writePinHigh(LED_e0_B);
    writePinHigh(LED_e1_R);
    writePinHigh(LED_e1_G);
    writePinHigh(LED_e1_B);
}
...
//    setPinOutput(LED_SCROLL_LOCK_PIN);
    setPinOutput(LED_e0_R);
    setPinOutput(LED_e0_G);
    setPinOutput(LED_e0_B);    
    setPinOutput(LED_e1_R);
    setPinOutput(LED_e1_G);
    setPinOutput(LED_e1_B);
...
__attribute__ ((weak)) bool rgb_matrix_indicators_kb(void) {
    if (rgb_matrix_is_enabled()) {  // turn the lights on when it is enabled.
        ...
        if (host_leds.scroll_lock) {
                rgb_matrix_set_color(4,100,100,100);
            }
...
led_config_t g_led_config = { // K20 Key Matrix & 31FL3731 KEYBOARD LED TEST array (Mechanic RGB layout) (made at SKYDIGITAL)
    {        // Key Matrix to LED Index
        ...
        // LED Index to Physical Position (for 32x8 ws2812 Square)
                    {  0, 18}, {  0, 36},
        { 14,  0}, { 14, 18}, { 14, 36}, { 14, 54},
        { 29,  0}, { 29, 18}, { 29, 36}, { 29, 54},
        { 43,  0}, { 43, 18}, { 43, 36},
        { 57,  0}, { 57, 18}, { 57, 36}, { 57, 54},
        { 71,  0},            { 71, 36},
